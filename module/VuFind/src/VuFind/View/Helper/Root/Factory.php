...
class Factory
{
     ...
     * @param ServiceManager $sm Service manager.
     *
     * @return Feedback If I wanted to only allow feedback from some libraries I could use my cookie method
     */
    public static function getFeedback(ServiceManager $sm)
    {
        $config = $sm->getServiceLocator()->get('VuFind\Config')->get('config');

        $mylib = $_COOKIE['mylib']; // mylibcloud
        $on = strlen($mylib);
        ...
        if ($on < 1) {
         ...
         $enabled = isset($config->$mylib->tab_enabled)
            ? $config->$mylib->tab_enabled : false;
       } 
    ...
    /**
     * Construct the oneclickReserve helper.
     *
     * @param ServiceManager $sm Service manager.
     *
     * @return [OneClickReserve] click_enabled = true or false
     */
    public static function oneClickReserve(ServiceManager $sm)
    {
        $config = $sm->getServiceLocator()->get('VuFind\Config')->get('config');

        $enabled = isset($config->OneClickReserve->click_enabled)
        ? $config->OneClickReserve->click_enabled : false;
    
        error_log(__FILE__ . " line " . __LINE__ . ' OneClickReserve click_enabled=' . $enabled ,0);

        return new ClickReserve($enabled);
    }
